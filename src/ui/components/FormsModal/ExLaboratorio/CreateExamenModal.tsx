...
export function CreateExamenModal({
  isOpen,
  onConfirm,
  onCancel,
}: CreateExamenModal) {
  ...
  const id = sessionStorage.getItem("id_paciente");
  ...
  async function confirm(e: any) {
    ...
    try {
      ...
      if (!data.laboratory || data.laboratory === "") {
        setAlerta({ type: "error", message: "El laboratorio no es válido" });
        throw new Error("El laboratorio no es válido");
      }
